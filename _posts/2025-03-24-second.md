...
2. **적절한 선택자 사용 (CSS Selector, XPath 등)**
   - 예제) `rvest` 패키지를 활용하여 특정 태그를 찾을 수 있음
```r
library(rvest)
...
3. **HTML의 속성을 활용**
- 링크(`<a>`), 이미지(`<img>`), 테이블(`<table>`), 폼(`<form>`) 등에서 데이터 추출 가능

# 2. Markdown이 중요한 이유
1. **Markdown 기반 웹사이트에서 데이터 크롤링**
   - GitHub, Notion, Jupyter Notebook 등에서 `.md` 파일 형식으로 제공된다.
...
3. **Markdown을 활용하면 GitHub 및 협업이 쉬워짐**
   - 수집한 데이터를 Markdown 문서로 변환하여 공유 가능
