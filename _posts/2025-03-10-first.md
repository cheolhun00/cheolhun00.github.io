---
layout: single
title:  "주사위 문제 해결하기"
---

#gpt 도움 없이 #팀 협업

[과제 1.1]
주사위 두 개를 굴려서 나온 눈의 수를 사용해서 두 자리 숫자를 크게 만드는 사람이 이기는 게임이다. 주사위를 처음 굴려서 나온 눈의 수를 10의 자리와 1의 자리 중 어느 곳에 사용할 지 먼저 결정해야 한다. 그런 다음 두 번째로 주사위를 굴려서 나온 눈의 수는 자동으로 남은 자리의 숫자로 사용된다.

예를 들어, 첫 번째 주사위가 3인데 10의 자리로 결정했고, 두 번째 6이 나왔다면 결과는 36이 된다.  

어떤 전략을 사용해야 게임에 이길 가능성이 가장 클까?

1. 위의 "two_dice.csv" 파일을 다운받아 최선이라고 선택한 전략의 총점수를 구하는 R 또는 파이썬 코드를 작성하여 제출하라. 최선의 전략을 사용할 때
  총점수 = 
  평균 = 

2. 최선의 전략일 때 확률분포를 구하고 그래프로 그려라.
 ![image](https://github.com/user-attachments/assets/bbed79ce-88b6-4e08-a6b3-978b64a02038)

4. 최선의 전략에서 기대 점수

```r
#과제 1.1 1번

X <- read.csv("C:/Users/hun-0/Desktop/캡스톤디자인/two_dice.csv")

head(X)
nrow(X)

result <-c(1:1000000)

#dice1이 3이하일 때와 4이상일 때
for(i in 1:nrow(X)){
  if (X[i,"dice1"] >= 4) {
  tenone <- 10*X[i,"dice1"] + X[i,"dice2"]
  result[i] <- tenone
  } else{
    tenone <- 10*X[i,"dice2"] + X[i,"dice1"]
    result[i] <- tenone
  }
}

sum(result)
mean(result)


#과제 1.1 2번
a <- c(11,12,13,14,15,16,
       21,22,23,24,25,26,
       31,32,33,34,35,36,
       41,42,43,44,45,46,
       51,52,53,54,55,56,
       61,62,63,64,65,66)

probability <- c(1:36)

Y <- data.frame(a,probability)

for (i in 1:6) {
  for(j in 1:6){
    Y[(6*i-5)+j-1,"probability"] <- sum(result==(10*i+j))/1000000
  }
}

barplot(Y[,"probability"]~Y[,"a"], main="최선전략 확률분포", ylab="확률",xlab="결과", col=rainbow(36),ylim=c(0.00,0.06)) #주사위는 이산형 확률변수이므로 막대그래프를 사용하였다.

#과제 1.1 3번
head(Y)
E <- sum(Y[,"a"]*Y[,"probability"])
print(E)
```
[과제 1.2]
주사위 3개를 굴려서 나온 눈의 수를 사용해서 세자리 숫자를 가장 크게 만드는 사람이 이기는 게임이다.
주사위를 처음 굴려서 나온 눈의 수를 세자리 숫자에서 어느 곳에 사용할 지 먼저 결정한다. 그런 다음 두 번째로 굴린 주사위에서 나온 눈의 수는 남은 두자리의 숫자 중 하나로 결정한다. 세 번째 나온 주사위 눈수는 자동으로 남은 숫자로 사용된다.

예를 들어, 첫 번째 주사위가 3인데 10의 자리로 결정했고, 두 번째 6이 나와 100의 자리 숫자에 사용하고, 세 번째 주사위 눈수가 1이면 결과는 631이 된다. 

어떤 전략이 최선의 전략인가?

1. 위의 "three_dice.csv" 파일을 다운받아 선택한 전략의 총점수를 구하는 R 또는 파이썬 코드를 작성하여 제출하라.
   총점수 = 
   평균 = 

2.최선의 전략일 때 확률분포를 구하고 그래프로 그려라.
![image](https://github.com/user-attachments/assets/5d9f86e4-3e71-4420-8001-4318a15c0722)


3. 최선의 전략에서 이론적인 기댓값

4. 이 게임에서 얻을 수 있는 일상생활이나 삶의 교훈이 무엇이라고 생각하는지 자신의 경험에 기반하여 300자 이상 기술하라. 다른 사람이 소감을 읽었을 때 생성형 AI를 이용했다는 생각이 든다면 평가 점수는 F이다.

```r
#과제 1.2 1번
X2 <- read.csv("C:/Users/hun-0/Desktop/캡스톤디자인/three_dice.csv")

head(X2)
nrow(X2)
result <- c(1:1000000)
result[]<- 0

#dice1이 1,2일 경우, 3,4일 경우, 5,6일 경우
 for(i in 1:nrow(X2)){
   if (X2[i,"dice1"] <= 2) {
     result[i] <- result[i] + 1*X2[i,"dice1"]
     if (X2[i,"dice2"] <= 3) {
       result[i] <- result[i] + 10*X2[i,"dice2"] + 100*X2[i,"dice3"]
     } else {
       result[i] <- result[i] + 100*X2[i,"dice2"] + 10*X2[i,"dice3"]
     }
   } else if (X2[i,"dice1"] <= 4) {
     result[i] <- result[i] + 10*X2[i,"dice1"]
     if (X2[i,"dice2"] <= 3) {
       result[i] <- result[i] + 1*X2[i,"dice2"] + 100*X2[i,"dice3"]
     } else {
       result[i] <- result[i] + 100*X2[i,"dice2"] + 1*X2[i,"dice3"]
   }
   } else {
     result[i] <- result[i] + 100*X2[i,"dice1"]
     if (X2[i,"dice2"] <= 3) {
       result[i] <- result[i] + 1*X2[i,"dice2"] + 10*X2[i,"dice3"]
     } else {
       result[i] <- result[i] + 10*X2[i,"dice2"] + 1*X2[i,"dice3"]
     }
   }
 }

sum(result)
mean(result)

#과제 1.2 2번
a <- c(1:216)
for(i in 1:6){
  for(j in 1:6){
    for(k in 1:6){
      a[36*i-36 + 6*j-6 + k] <- 100*i + 10*j + k
    }
  }
}

probability <- c(1:216)
probability[] <- 0

Z <- data.frame(a,probability)
Z
for(i in 1:6){
  for(j in 1:6){
    for(k in 1:6){
      Z[36*i-36 + 6*j-6 + k,"probability"] <- sum(result==(100*i+10*j+1*k))/1000000
    }
  }
}


barplot(Z[,"probability"]~Z[,"a"], main="최선전략 확률분포", ylab="확률",xlab="결과", col=rainbow(36))


#과제 1.2 3번
theory.prob <- c(1:216)
theory.prob[] <- 0
ZZ <- data.frame(a, theory.prob)

#case1-1: dice1이 1,2이고 dice2가 1,2,3일 경우
i <- c(1:6) #백의 자리
j <- c(1,2,3) #십의자리
k <- c(1,2) #일의 자리

for (m in 1:2){
  for (n in 1:3){
    for (l in 1:6){
      result <- c(k[m] + 10*j[n] + 100*i[l])
      for(h in 1:216)
        if (result==ZZ[h,"a"]){
          ZZ[h,"theory.prob"] <- ZZ[h,"theory.prob"] + 1/216
        }
    }
  }
}


#case1-2: dice1이 1,2이고 dice2가 4,5,6일 경우
i <- c(4,5,6)
j <- c(1:6)
k <- c(1,2)

for (m in 1:2){
  for (n in 1:6){
    for (l in 1:3){
      result <- c(k[m] + 10*j[n] + 100*i[l])
      for(h in 1:216)
        if (result==ZZ[h,"a"]){
          ZZ[h,"theory.prob"] <- ZZ[h,"theory.prob"] + 1/216
        }
    }
  }
}

#case2-1: dice1이 3,4이고 dice2가 1,2,3일 경우
i <- c(1:6)
j <- c(3,4)
k <- c(1,2,3)

for (m in 1:3){
  for (n in 1:2){
    for (l in 1:6){
      result <- c(k[m] + 10*j[n] + 100*i[l])
      for(h in 1:216)
        if (result==ZZ[h,"a"]){
          ZZ[h,"theory.prob"] <- ZZ[h,"theory.prob"] + 1/216
        }
    }
  }
}

#case2-2: dice1이 3,4이고 dice2가 4,5,6일 경우
i <- c(4,5,6)
j <- c(3,4)
k <- c(1:6)

for (m in 1:6){
  for (n in 1:2){
    for (l in 1:3){
      result <- c(k[m] + 10*j[n] + 100*i[l])
      for(h in 1:216)
        if (result==ZZ[h,"a"]){
          ZZ[h,"theory.prob"] <- ZZ[h,"theory.prob"] + 1/216
        }
    }
  }
}

#case3-1: dice1이 5,6이고 dice2가 1,2,3일 경우
i <- c(5,6)
j <- c(1:6)
k <- c(1,2,3)

for (m in 1:3){
  for (n in 1:6){
    for (l in 1:2){
      result <- c(k[m] + 10*j[n] + 100*i[l])
      for(h in 1:216)
        if (result==ZZ[h,"a"]){
          ZZ[h,"theory.prob"] <- ZZ[h,"theory.prob"] + 1/216
        }
    }
  }
}

#case3-2: dice1이 5,6이고 dice2가 4,5,6일 경우
i <- c(5,6)
j <- c(4,5,6)
k <- c(1:6)

for (m in 1:6){
  for (n in 1:3){
    for (l in 1:2){
      result <- c(k[m] + 10*j[n] + 100*i[l])
      for(h in 1:216)
        if (result==ZZ[h,"a"]){
          ZZ[h,"theory.prob"] <- ZZ[h,"theory.prob"] + 1/216
        }
    }
  }
}

sum(ZZ[,"theory.prob"])

E2 <- sum(ZZ[,"a"]*ZZ[,"theory.prob"])
print(E2)


4. 어떤 일이나 프로젝트를 수행할 때 어느 한 단계에서 여러 의사결정을 할 수 있는 선택지가 주어졌을 때,
각각의 선택에 따라 다음 단계의 의사결정을 할 때도 영향을 미치는 것을 알 수 있다. 첫 번째 문제에서의
최선의 전략은 두 번째 문제의 최선의 전략이 되지 못했다. 이는 기존 문제에 새로운 상황이 발생하였을 때
기존 전략이 통하지 않을 수 있고 새로운 전략을 조건에 맞게 다시 수정해야했다. 따라서 상황에 맞춰서
전략을 수정하며 최선의 결과를 이끌어 내도록 노력해야 함을 배웠다. 한편으로, 최선의 전략이 항상 좋은
결과를 가지고 오는 것은 아니고, 때로는 도전적인 선택을 하더라도 최고의 결과를 얻어야 할 때도 있으므로
유연한 사고 또한 중요하다고 느꼈다.
```

느낀점: 과제를 수행하며 gpt를 사용하지 않고 혼자 힘으로 해결해보았지만,
        다른 팀원들과 함께 풀이과정과 아이디어들을 공유해보니, gpt를 사용하였을 때 생산성이 월등히
        높았고 정확도 또한 매우 높았다. 그리고 R코드를 직접 짜서 결과를 도출해냈는데 그 결과는 다른 팀원과
        동일하였지만 코드의 간결성 방면에서 많이 뒤쳐져있었다. 그러나, gpt가 내놓은 답변을 바로 이해하기는
        쉽지 않았다. 따라서, gpt를 조력자로 삼고 gpt가 내놓는 답변을 비판적으로 받아들여 추가적인 질문과 답변을
        통해서 더욱 좋은 결과를 도출해낼 수 있도록 노력해야겠다.
        답변을 그대로 사용하지 않고 
